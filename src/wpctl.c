...
#define DEFAULT_AUDIO_SINK_ID ((int64_t)1 << 32)
#define DEFAULT_AUDIO_SOURCE_ID ((int64_t)1 << 33)
#define DEFAULT_VIDEO_SOURCE_ID ((int64_t)1 << 34)



static gboolean
parse_id (gboolean allow_def_audio, gboolean allow_def_video, gchar *arg, int64_t *result_id, GError **error)
{
...
static gboolean
translate_id (WpPlugin *def_nodes_api, int64_t id, int32_t *res, GError **error)
{
...
static gboolean
run_nodes_by_pid (WpObjectManager *om, int32_t pid,
    WpIteratorFoldFunc func, gpointer data)
{
  ...
  for (; wp_iterator_next (client_it, &client_val); g_value_unset (&client_val)) {
    ...
    int32_t client_id = wp_proxy_get_bound_id (WP_PROXY (client));
...
{
  WpCtl *self;
  int32_t default_node;
  WpPlugin *mixer_api;
  GHashTable *printed_filters;
};

static void
print_controls (int32_t id, struct print_context *context)
{
...
print_device (const GValue *item, gpointer data)
{
  ...
  int32_t id = wp_proxy_get_bound_id (WP_PROXY (obj));
...
print_filter_node (const GValue *item, gpointer data)
{
  ...
  for (; wp_iterator_next (it, &val); g_value_unset (&val)) {
    ...
    int32_t id = wp_proxy_get_bound_id (WP_PROXY (node));
...
set_route_run (WpCtl * self)
{
  ...
  int32_t route_device = atoi (route_device_str);
  ...
  int32_t device_id = atoi (device_id_str);
