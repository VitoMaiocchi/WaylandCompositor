...
namespace Output {
	...
	void get_volume_run(WpData* wpd) {
	...
	void wireplumber_run_mainloop() {
		...
		g_signal_connect_swapped (wpd.om, "installed",
			(GCallback) get_volume_run, &wpd);

		mainloop_running = true;
		while(mainloop_running) g_main_context_iteration(g_main_loop_get_context(wpd.loop), true);
		info("wireplumber mainloop stopped");
	}

	void setupPipeWire() {
		wireplumber_run_mainloop();
	}
